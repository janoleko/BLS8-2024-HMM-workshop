...
Another option for checking the model fit is to simulate data from the estimated model and compare it to the observed data. If the model describes the true data well, these two should not deviate substantially.
...
ggplot(head(data_hmm, 500), 
       aes(Time, logODBA, col = viterbi_3s, group = NA)) +
  geom_line() +
  scale_color_manual(values = pal, name = "State")

# Plotting simulated data coloured by states
ggplot(head(simdata, 500), 
       aes(Time, logODBA, col = factor(states), group = NA)) +
  geom_line() +
  scale_color_manual(values = pal, name = "State")
```

Overall, our model looks good. We only see minor issues with state 3, as in the real data some sojourn times are very long while most are very short and in the simulated data this is more homogeneous.

We can also visualize the marginal distribution implied by our model:
...
Lastly, we can investigate the autocorrelation function of the real and simulated data.
